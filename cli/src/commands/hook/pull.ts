import {Args} from '@oclif/core'
import {mkdir, writeFile} from 'node:fs/promises'
import {dirname, join} from 'node:path'

import {LoopressCommand} from '../../lib/base.js'
import {basenameKey, findOrphanedFiles} from '../../lib/find-orphaned-files.js'
import {pluralize} from '../../utils/pluralize.js'

type HookFile = {
  content: string
  filename: string
}

type PullResult = {
  orphans: string[]
  pulled: string[]
  status: 'dry-run' | 'success'
}

export default class Pull extends LoopressCommand {
  static args = {
    path: Args.string({description: 'Path to hooks directory (overrides project config)'}),
  }

  static description = 'Pull hook files (actions, filters, cron) from WordPress'
  static enableJsonFlag = true
  static examples = ['$ lps hook pull', '$ lps hook pull --path ./hooks']
  static flags = {
    ...LoopressCommand.dryRunFlag,
    ...LoopressCommand.yesFlag,
  }

  async run(): Promise<PullResult> {
    const {args} = await this.parse(Pull)
    const {url} = this.siteConfig
    const path = this.resolveHooksPath(args.path)

    this.log(`Pulling hooks from ${url}`)
    this.log(`Hooks path: ${path}`)

    const files = await this.wp.get<HookFile[]>('loopress/v1/hook-files')

    // A `<filename>.php` no longer present remotely belongs to a file deleted on WordPress
    // (push itself stays additive-only, but pull already cleans up locally, same as
    // `api pull`/`snippet pull`).
    const orphans = await findOrphanedFiles(path, new Set(files.map((file) => file.filename)), {
      extensions: ['.php'],
      key: basenameKey,
      recursive: true,
    })

    const pulled = files.map((file) => file.filename)

    await this.pullDirectory(path, files, orphans, {
      alwaysCreateDir: true,
      dryRunMessage: `Would pull ${pluralize(files.length, 'hook file')} to ${path}`,
      orphanReason: 'no longer present on WordPress',
      pulledMessage: `Pulled ${pluralize(files.length, 'hook file')} to ${path}`,
      title: (file) => file.filename,
      async write(file, writeDir) {
        const filePath = join(writeDir, `${file.filename}.php`)
        // filename can contain '/' (a nested slug, e.g. content/filters): writeFile() doesn't
        // create parent directories on its own the way mkdir()'s recursive option does above
        // for the top-level path.
        await mkdir(dirname(filePath), {recursive: true})
        await writeFile(filePath, file.content)
      },
    })

    if (this.dryRun) return {orphans, pulled, status: 'dry-run'}

    return {orphans, pulled, status: 'success'}
  }
}
