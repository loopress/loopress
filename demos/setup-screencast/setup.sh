...
if [[ "$OS" == Darwin ]]; then
  ...
  $SUDO env DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
    ffmpeg fonts-jetbrains-mono asciinema curl ca-certificates
  if ! have google-chrome && ! have google-chrome-stable; then
    ...
    $SUDO env DEBIAN_FRONTEND=noninteractive apt-get install -y -qq "$deb"
    rm -f "$deb"
  fi
...
echo "== node deps (node-pty + playwright) =="
