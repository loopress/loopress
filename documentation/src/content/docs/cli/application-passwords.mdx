...
The Loopress CLI authenticates with WordPress using [Application Passwords](https://make.wordpress.org/core/2020/11/05/application-passwords-integration-guide/), a feature built into WordPress since version 5.6. You generate one under **Users → Profile → Application Passwords** and pass it to `lps project config`.

Some security plugins and hosting providers disable this feature by default. If the Application Passwords section is missing from your profile page, or if the CLI returns a `401` authentication error, one of the configurations below is likely the cause.
