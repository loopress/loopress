...
| Menu sync (nav menus and the active theme's menu locations) | ✓ | ✓ |
| `lps menu pull` / `push` / `list` / `diff` | ✓ | ✓ |
| Option tracking (individual `wp_options` rows you choose) | ✓ | ✓ |
| `lps option add` / `pull` / `push` / `list` / `remove` / `diff` | ✓ | ✓ |
| Theme styles (block theme Global Styles, through WordPress core's own REST API) | ✓ | ✓ |
...
| Hooks (WordPress actions, filters and cron jobs deployed from PHP files) | | ✓ |
| `lps hook pull` / `push` / `list` / `rm` | | ✓ |
| Static pages (hand-written HTML pages rendered by the theme) | | ✓ |
| `lps page push` / `list` / `diff` | | ✓ |
...
- **Static pages**: push hand-written HTML files as WordPress pages with `lps page push`, rendered inside the active theme and locked against edits in wp-admin, see [Static Pages](/pages/)
