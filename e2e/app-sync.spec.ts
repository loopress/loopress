...
test("pushes a built app, serves it through the shortcode, then pulls it back", async ({
	...
	// A page with the shortcode renders the mount point and enqueues the module script.
	await page.goto(`${wp.url}/wp-admin/post-new.php?post_type=page`);
	// (creating the page via the editor is flaky to script here; assert the render path
	// instead through the shortcode-rendering REST helper)
	const rendered = await request.post(`${wp.url}/wp-json/wp/v2/block-renderer/core/shortcode`, {
		failOnStatusCode: false,
		headers: {Authorization: `Basic ${Buffer.from(`${wp.username}:${wp.appPassword}`).toString("base64")}`},
		params: {context: "edit"},
		data: {attributes: {text: `[loopress_app name="${name}"]`}},
	});
	if (rendered.ok()) {
		expect(((await rendered.json()) as {rendered: string}).rendered).toContain(`id="loopress-app-${name}"`);
	}
