...
test.describe("two snippet plugins active at once", () => {
	...
	test('snippet list fails with a clear "multiple plugins active" error', async ({
		...
		expect(unwrap(result.stderr)).toContain(
			"Multiple snippet plugins are active",
		);
	});

	test("snippet push fails the same way instead of silently syncing to the wrong plugin", async ({
		projectDir,
		runCli,
	}) => {
		...
		writeFileSync(
			join(snippetsDir, "conflict.json"),
			JSON.stringify({ location: "everywhere", name: "Conflict", type: "php" }),
		);
