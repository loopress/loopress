...
import {LpsError, runLps} from './run-lps.js'
...
export async function runMutatingTool(tool: string, args: string[], confirmToken?: string): Promise<MutatingToolResult> {
  if (!confirmToken) {
    // oclif expects the topic/command first; the flag has to come after it, not before.
    const preview = await runLps([...args, '--dry-run'])
  ...
  const result = await runLps(consumed.args)
