...
// `lps push`/`lps pull` fan out to ten resource commands in sequence, each doing its own
...
export function registerBulkTools(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        'Push every local resource (plugins, composer, ACF, API routes, hooks, forms, pages, SEO, menus, options, snippets) to WordPress in one run, the equivalent of `lps push`.' +
        PREVIEW_SUFFIX,
  ...
  server.registerTool(
    ...
    {
      description:
        'Pull every resource (plugins, composer, ACF, API routes, hooks, forms, SEO, menus, options, snippets) from WordPress into local files in one run, the equivalent of `lps pull`.',
