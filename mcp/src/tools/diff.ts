...
const RESOURCES = ['snippet', 'form', 'acf', 'api', 'hook', 'seo', 'option', 'theme-styles', 'composer'] as const
...
export function registerDiffTools(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        'Show what differs between local tracked files and a WordPress environment, or between two environments. Covers snippets, forms, ACF, API routes, hooks, SEO, options, theme styles and Composer. Plugins and themes have their own plugin_status / theme_status.',
      inputSchema: {
        against: z
          .string()
          .optional()
