...
- **Loopress Full** (`loopress-full.zip`): everything in Loopress Light, plus code snippet sync (`src/Snippets/`), Composer dependency management (`src/Dependencies/`), custom API routes (`src/Api/`), and single-page app hosting (`src/Apps/`). Distributed only from loopress.dev, never wordpress.org. Ships with `Update URI: https://loopress.dev` so wordpress.org can never push an update over it, and deactivates Loopress Light on activation (the two editions never run side by side; both define `LOOPRESS_VERSION` and refuse to boot if it is already defined). `src/Apps/` follows the same wordpress.org rejection precedent as `src/Snippets/` and `src/Api/`: a bundled SPA runs on the site's own origin in every visitor's browser.
