...
function deriveFullOnlyFeatures(root) {
  ...
  throw new Error('Could not find the Plus feature list in loopress.php')
}
...
// Guarded so the file can be `require()`d (e.g. from tests, to reuse deriveFullOnlyFeatures)
// without triggering a full build as a side effect.
if (require.main === module) {
  ...
  // their own frontend/full/ or frontend/light/ folder); wp-scripts derives the output
  // name from the entry file's basename without its extension, so it always writes
  // build/index.js regardless of which one was just built: the shared PHP
  ...
  fs.copyFileSync(
    ...
    path.join(stageDir, 'build', 'index.js'),
  )
  fs.copyFileSync(
    path.join(root, 'build', 'index.asset.php'),
    path.join(stageDir, 'build', 'index.asset.php'),
  )
...
module.exports = { deriveFullOnlyFeatures }
