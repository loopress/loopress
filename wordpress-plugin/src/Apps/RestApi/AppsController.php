...
class AppsController
{
    ...
    public function commit(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!is_array($body)) {
            return new WP_REST_Response(['error' => 'Expected a JSON manifest body'], 400);
        }

        try {
            $manifest = AppManifest::normalize($body);
        ...
        $this->store->put($name, [
            ...
            'previousManifest' => is_array($previous['files'] ?? null) ? $previous['files'] : [],
            'deployedAt'       => gmdate('c'),
        ]);
